...
func TestMissChange(t *testing.T) {
	...
	DPrintf("--- Start Section 0 ---\n")
	cfg.StartServer(0, 0)
	cfg.StartServer(1, 0)
	cfg.StartServer(2, 0)

	DPrintf("--- Append Section 2 ---\n")
	for i := 0; i < n; i++ {
		check(t, ck, ka[i], va[i])
		x := randstring(20)
		DPrintf("--- Append %v ---\n", i)

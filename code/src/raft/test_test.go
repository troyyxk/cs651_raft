...
func TestFailAgree2B(t *testing.T) {
	...
	// todo, why the disconnected ones still running
	DPrintf("<------------------------- disconnect: start ------------------------->\n")
	cfg.disconnect((leader + 1) % servers)
	DPrintf("<------------------------- disconnect: end ------------------------->\n")
	DPrintf("<------------------------- leader: %v------------------------->\n", leader)
	...
	// re-connect
	DPrintf("<------------------------- reconnected: start ------------------------->\n")
	cfg.connect((leader + 1) % servers)
	DPrintf("<------------------------- reconnected: end ------------------------->\n")
...
func TestBackup2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	DPrintf("<------------------------- leader1: %v------------------------->\n", leader1)
	DPrintf("<-------------------------Disconnect 3 nodes------------------------->\n")
	cfg.disconnect((leader1 + 2) % servers)
	cfg.disconnect((leader1 + 3) % servers)
	cfg.disconnect((leader1 + 4) % servers)

	DPrintf("<-------------------------Submit command wont commit 1------------------------->\n")
	...
	DPrintf("<-------------------------Disconnect the 2 running nodes, connect the 3 disconected nodes------------------------->\n")
	...
	DPrintf("<-------------------------Submit command that will commit 1------------------------->\n")
	...
	DPrintf("<-------------------------Submit command wont commit 2------------------------->\n")
	...
	DPrintf("<-------------------------Connect leader 1, original 1 with leader 1 with no logs suppose and other; disconnect rest------------------------->\n")
	...
	DPrintf("<-------------------------Submit command that will commit 2------------------------->\n")
	DPrintf("<-------------------------Participants: %v, %v, %v------------------------->\n", (leader1+0)%servers, (leader1+1)%servers, other)
	// lots of successful commands to new group.
	for i := 0; i < 50; i++ {
		cfg.one(rand.Int(), 3, true)
	}

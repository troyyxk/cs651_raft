...
func (cfg *config) checkConsistency(server int, index int, logEntry logEntry) {
	for j := 0; j < len(cfg.logs); j++ {
		...
		if ok && (e != logEntry) {
			//DPrintf("sever %v, status: %v\n", server, cfg.rafts[server].status)
...
func (cfg *config) apply(server int, m ApplyMsg) {
	...
	//DPrintf("CCC< node %v, term %v, status %v, node %v: apply(), before: m.CommandIndex: %v, cfg.nextIndex[server]: %v",
	//	cfg.rafts[server].me, cfg.rafts[server].currentTerm, cfg.rafts[server].status, cfg.rafts[server].me,
	...
	//DPrintf("CCC< node %v, term %v, status %v: apply(), after: m.CommandIndex: %v, cfg.nextIndex[server]: %v",
	//	cfg.rafts[server].me, cfg.rafts[server].currentTerm, cfg.rafts[server].status,
	//	m.CommandIndex, cfg.nextIndex[server])
...
func (cfg *config) applySnap(server int, m ApplyMsg) {
	if m.SnapshotValid {
		...
		//DPrintf("CCC< node %v, term %v, status %v: submitting an entry m.CommandIndex: %v, m.Command: %v, cfg.numCommands[server]: %v",
		//	cfg.rafts[server].me, cfg.rafts[server].currentTerm, cfg.rafts[server].status,
		//	m.CommandIndex, m.Command, cfg.numCommands[server])
...
func (cfg *config) checkOneLeader() int {
	for iters := 0; iters < 10; iters++ {
		...
		for i := 0; i < cfg.n; i++ {
			//DPrintf("CCC= node%v, connected: %v\n", i, cfg.connected[i])
			if cfg.connected[i] {
...
func (cfg *config) nCommitted(index int) (int, logEntry) {
	...
	for i := 0; i < len(cfg.rafts); i++ {
		cfg.mu.Lock()
		e, ok := cfg.logs[i][index]
		// todo, why start at 1 and need 1 extra log entry at front
	...
	for time.Since(t0).Seconds() < 10 {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				nd, e := cfg.nCommitted(index)
				//DPrintf("TTT= index: %v, nd: %v, expectedServers: %v\n", index, nd, expectedServers)
			...
			if retry == false {
				log.Fatalf("one(%v), type 0, failed to reach agreement", cmd)
			}
		} else {
			time.Sleep(50 * time.Millisecond)
		}
	}
	log.Fatalf("one(%v), type 1, failed to reach agreement", cmd)
	return -1
}
